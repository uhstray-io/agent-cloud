...
| Collect Service Conformance | `collect-service-conformance.yml` | The service deployment workflow's ONLY status writer, every 15 minutes: each workflow template's own Semaphore history (its newest 1000 tasks; a template that fills that window is reported as `history_window_full`) → newest result per service and step (a failure that recorded none counts as `fail` with its last 20 lines) → NetBox VM custom fields with a scoped view/change-VM token, MERGED into the status NetBox already holds so a result older than the window is kept, not erased, and Loki for the `service-conformance` Grafana dashboard. Its dry run is the read-only failure report |
