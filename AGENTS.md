...
| Inspect Host Containers | `inspect-host-containers.yml` | Read-only: every container on one host, per engine (rootless podman, rootful podman, Docker), with image, state, restart policy and compose working directory. For onboarding a host that has no service group yet. Has a `(Dev)` copy |
