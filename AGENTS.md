...
- `platform/services/agentgateway/context/architecture.md` — agentgateway inference edge gateway (per-client API keys as sha256 hashes, per-key hourly token budgets backed by its own Postgres, one global request bucket, `custom` completions upstream; local-dev fronts LM Studio, prod fronts vLLM on the DGX Spark head; own VM, admin port never published)
