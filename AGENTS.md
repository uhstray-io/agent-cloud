...
| Seed o11y Alert Webhook | `seed-o11y-alert-webhook.yml` | From the reviewed controller checkout, read the bot token from OpenBao, reconcile one bot-owned webhook in the privately declared text channel, and merge its URL into `secret/services/o11y`; the Dev template requires the exact commit SHA |
