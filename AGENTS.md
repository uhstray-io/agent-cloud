...
| Lookup Service Inventory / Validate Address Free | `lookup-service-inventory.yml` / `validate-address-free.yml` | Workflow steps 2 and 3, before provisioning: the declared spec, IPAM status and address claims (read-only), then the pfSense ARP check and the NetBox VM record (created `planned` in the discovery-maintained Proxmox cluster, `vm-recorder` token: view/add VMs, view clusters). Both resolve the declared VM through `vars/declared-vm.yml`, provision-vm's precedence |
| Verify Service Persistence | `verify-service-persistence.yml` | Workflow step systemd-enablement: restart policy `always` (or `"no"` for a one-shot init container, the compose guard's set; podman's boot unit starts nothing else) and rootless-podman linger plus its boot unit; an empty container selection fails |
