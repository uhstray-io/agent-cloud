...
| Allocate NetBox IP | `netbox-allocate-ip.yml` | Ask the IPAM authority for free addresses and report the recorded state of named ones. Read-only unless `-e reserve=true`; reserving takes EXPLICIT static addresses and first verifies the live pfSense DHCP boundary over trusted HTTPS — never "the next free one", which two runs a minute apart would resolve differently |
