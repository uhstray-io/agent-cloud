...
| `secret/services/agentgateway` | Inference gateway: `vllm_api_key` (the upstream credential the gateway holds; seeded, never generated), `agw_db_password` (its own budget Postgres) + one `client_<name>` API key per enrolled identity (generated once by the deploy, reused; rotation is explicit) |
