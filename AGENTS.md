...
| Deploy agentgateway | `deploy-agentgateway.yml` | Inference edge gateway (podman, gateway + its own internal-only Postgres for per-key token budgets): secrets (upstream `vllm_api_key` is `existing` — never generated; one `client_<name>` key per inventory identity, generated once and reused) → render `.env` + `config.yaml` (identities as sha256 hashes, no plaintext key) → deploy.sh (readiness probed from the host; the image has no shell) → verify: readiness 200, unauthenticated `/v1/models` 401, one enrolled identity lists the models |
