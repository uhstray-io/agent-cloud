...
- `platform/services/agentgateway/context/architecture.md` — agentgateway inference edge gateway (per-client API keys as sha256 hashes, per-key hourly token budgets backed by its own Postgres, one global request bucket, `custom` completions upstream; built-in operator UI on its own listener at `admin.inference.<zone>`, authenticated by the gateway's own OIDC policy against Authentik (admin group required), Caddy a plain proxy; local-dev fronts LM Studio, prod fronts vLLM on the DGX Spark head; own VM, admin port never published)
...
| `secret/services/netbox` | All NetBox secrets (DB, Redis, Diode, Hydra, superuser, orb-agent creds with timestamp), plus one scoped API token per identity `Provision NetBox Automation Token` mints: `automation_api_token` (device-writer), `collector_api_token` (workflow-collector: view/change VMs), `vm_recorder_api_token` (vm-recorder: view/add VMs, view clusters) |
...
| `secret/services/discord` | `bot_token`, the bot credential used by the Dev-bound o11y webhook reconciliation workflow; never a Grafana runtime secret |
| `secret/services/o11y` | Grafana admin password and the `alert_discord_webhook_url` produced from the declared Discord text channel; the URL is read only when alerts are enabled |
...
| `secret/services/agentgateway` | Inference gateway: `vllm_api_key` (the upstream credential the gateway holds; seeded, never generated), `agw_db_password` (its own budget Postgres), `agw_oidc_cookie_seed` (hashed into the UI session cookie key) + one `client_<name>` virtual key per enrolled identity (generated once by the deploy, reused; rotate/revoke via `Manage agentgateway Client Key`). The UI's OIDC client secret is Authentik's (`agentgateway_oidc_client_secret`, shared-read) |
...
| Recover NetBox Runtime (Dev) | `recover-netbox-runtime.yml` | Default read-only preflight checks the existing Compose bytes, image IDs, mounts, environment, and restart policy; explicit apply starts or recreates only the four core containers through Compose, then verifies health and login without replacing named volumes |
...
| Recover Authentik Audit Runtime (Dev) | `recover-authentik-audit-runtime.yml` | Default read-only preflight verifies the existing containers and exact database/cache data volumes; explicit apply starts only the existing database, cache, and server, leaving the blueprint worker stopped until the retirement account audit |
...
| Provision NetBox Automation Token | `provision-netbox-automation-token.yml` | One-time per identity: mint a scoped NetBox v2 API token via the Django shell and store it in `secret/services/netbox` (idempotent; never printed). The identity is the **Token profile** survey field: blank or `device-writer` → `automation_api_token`, `workflow-collector` → `collector_api_token`, `vm-recorder` → `vm_recorder_api_token`; an unknown name is refused before anything is minted |
| Deploy agentgateway | `deploy-agentgateway.yml` | Inference edge gateway (podman, gateway + its own internal-only Postgres for per-key token budgets): secrets (upstream `vllm_api_key` is `existing` — never generated; one `client_<name>` key per inventory identity, generated once and reused) → render `.env` + `config.yaml` (identities as sha256 hashes, no plaintext key) → deploy.sh (readiness probed from the sibling db container over the compose network; the gateway image has no shell) → verify: readiness 200, unauthenticated `/v1/models` 401, then as one enrolled identity (a `uri` header from the target to the published port, never any process's argv) `/v1/models` lists every model it may use and one chat completion comes back from the upstream; a 429 from the gateway's own request bucket fails the deploy as unproven unless `-e agw_verify_allow_unproven=true` accepts it (skipped under `--check`) |
| Clean Deploy agentgateway | `clean-deploy-agentgateway.yml` | Remove both containers, the budget-usage volume + rendered files and redeploy; keys come back from OpenBao unchanged, every identity's budget window restarts at zero |
| Manage agentgateway Client Key | `manage-agentgateway-client-key.yml` | Rotate or revoke ONE virtual key (client identity) as code: `-e client=<name> -e action=rotate` (name must be in `agw_clients`) merges a new value via the shared merge task; `action=revoke` (name must already be REMOVED from `agw_clients`, or the next deploy would mint it back) deletes the field with a KV-v2 merge-patch null; both end by importing the deploy, which re-renders the hashes and reloads. Adding a client is an inventory change + deploy; handout is `Back Up Credentials to site-config` with `credential_fields=client_<name>`. Never prints a value |
...
| Seed Postiz Secrets | `seed-postiz-secrets.yml` | Additively place the operator's social-platform credentials at `secret/services/postiz` (KV-v2 merge-patch). Runs in its own **isolated** Semaphore environment: `scripts/postiz-seed-input.py` stages each value as an encrypted `SEED_*` input, runs one task and removes the inputs — never a survey var or launch-time extra var, both of which Semaphore persists. `postiz_verify_access_only=true` is the read-only access check |
...
| Allocate NetBox IP | `netbox-allocate-ip.yml` | Ask the IPAM authority for free addresses and report the recorded state of named ones. Read-only unless `-e reserve=true`; reserving takes EXPLICIT static addresses and first verifies the live pfSense DHCP boundary over HTTPS (certificate validation on unless private NetBox inventory explicitly disables it) — never "the next free one", which two runs a minute apart would resolve differently |
| Resize VM | `resize-vm.yml` | Converge a live VM's cores/memory/disk, and its Proxmox guest-agent option (`agent=1`, opt-out `vm_agent: false`), to the spec declared in `site-config/proxmox/vm-specs.yml` (grow-only disk, opt-in reboot; a run without `allow_reboot` is a safe diff preview) |
...
| Seed OpenBao Key | `seed-openbao-key.yml` | Idempotently merge ONE key/value into a secret path (created if absent, siblings preserved) — code-managed placement of a shared secret a reader deploy needs (e.g. honcho's `secret/services/nemoclaw:gemini_api_key`) Runs in its own **isolated** Semaphore environment: stage the value with `scripts/semaphore-seed-input.py` (encrypted input, one task, input removed), never by hand or with a one-off script |
| Provision Seed Environment | `provision-seed-environment.yml` | Give ONE seed template that declares `isolated_environment` in `templates.yml` (Seed OpenBao Key, Seed Postiz Secrets) its own environment with an encrypted copy of the controller AppRole, then bind it. Preserves existing authentication, never rotates it. Follow with the seed CLI's `--verify-only` read-only check before the first real seed |
| Manage Semaphore Access | `manage-semaphore-access.yml` | Hold every identity that can launch a task to a declaration, because launch rights are runner rights (a templated extra var runs code on the runner): the project team against private `semaphore_project_members` (username to role), system admins against `semaphore_admin_users`, and integrations against none. Check mode reports names and roles only; a real run adds, re-roles and removes members, then reads the team back. An undeclared admin or any integration fails the run by name for an operator decision |
| Seed o11y Alert Webhook | `seed-o11y-alert-webhook.yml` | From an exact reviewed, clean controller checkout, read the bot token from OpenBao, reconcile one bot-owned webhook in the privately declared text channel, and merge its URL into `secret/services/o11y`; every run requires the exact commit SHA |
| Manage Caddy Sites | `manage-caddy-sites.yml` | **Read** the live Caddyfile (reports every site block, its upstreams, and whether inventory or a hand edit owns it), insert/update the marked block, optionally **retire** hand-maintained blocks (`caddy_retire_sites`) so inventory can adopt that hostname; validate + restart. Has a `(Dev)` variant: `main` still carries the insert-only version, so a route lands through the read/retire/rescue code before promotion. A route that fronts an API the upstream only partly authenticates (vLLM: `--api-key` covers `/v1`, not `/metrics`, `/pause`, `/abort_requests`) is declared in the **allowlist shape** — `handle /v1/*` with an in-block `respond 401` for a missing Bearer header, `handle /health`, bare `handle { respond 404 }` — which `Caddyfile.local.j2` renders locally from a route's `inference_api: true` against `scripts/fake-inference-upstream.py` |
...
| Collect Service Conformance | `collect-service-conformance.yml` | The service deployment workflow's ONLY status writer, every 15 minutes: each workflow template's own Semaphore history (its newest 1000 tasks; a template that fills that window is reported as `history_window_full` and marks the services it can hide as `history_incomplete`) → newest result per service and step (a failure that recorded none counts as `fail` with its last 20 lines), with each VM's status already in NetBox merged underneath as `retained` so a result older than the window stays in NetBox, the report and Loki alike → NetBox VM custom fields with a scoped view/change-VM token, and Loki for the `service-conformance` Grafana dashboard. Its dry run is the read-only failure report. Has a `(Dev)` copy, launched by hand and never scheduled (a variant never inherits the base schedule), which runs the collector in production from dev before promotion; first production run 2026-09-25 (Semaphore task 1341) updated the workflow custom fields on seven existing VM records |
...
| Lookup Service Inventory / Validate Address Free | `lookup-service-inventory.yml` / `validate-address-free.yml` | Workflow steps 2 and 3, before provisioning: the declared spec, IPAM status and address claims (read-only), then the pfSense ARP check and the NetBox VM record (created `planned` in the discovery-maintained Proxmox cluster, `vm-recorder` token: view/add VMs, view clusters). An address in ARP counts as the service's own only when every MAC seen for it is on a NIC of the declared VM, running on Proxmox; an existing record must be the one record with that name and sit in a Proxmox cluster discovery maintains. Both resolve the declared VM through `vars/declared-vm.yml`, provision-vm's precedence |
| Verify Service Persistence | `verify-service-persistence.yml` | Workflow step systemd-enablement: restart policy `always` (podman's boot unit starts nothing else), or `"no"` only from a container that declares itself one-shot (label `agent-cloud.one-shot: "true"`, required by the compose guard) and has exited 0; rootless-podman linger plus its boot unit; an empty container selection fails |
| Verify Service Health | `verify-service-health.yml` | Workflow step service-validate: HTTP 200 from the declared `health_url`, or `service_url` + `health_path` (the fleet overview's `health_status_codes` is not read). Probed from the executor, unless the host sets `health_probe_on_host: true`: then from the host itself against a loopback `health_url`, for a service published on loopback only or firewalled to the Caddy host, which the executor cannot reach however healthy it is |
| Ensure Service Persistence | `ensure-service-persistence.yml` | The SETUP half of systemd-enablement: rootless podman gets linger plus podman's user boot unit (`tasks/enable-linger.yml`), rootful podman the system `podman-restart.service`, Docker `docker.service`. Boot settings only; restarts nothing, so it is safe on a running service, Semaphore's own host included. A container with a policy other than `always` is fixed by rerunning its service's deploy. Has a `(Dev)` copy |
| Inspect Host Containers | `inspect-host-containers.yml` | Read-only: the containers on exactly one host, per engine (rootless podman for the connecting user and any declared `linger_user`, rootful podman, Docker), with image, state, restart policy and compose working directory. For onboarding a host that has no service group yet. Has a `(Dev)` copy |
| Inspect Service Runtime | `inspect-service-runtime.yml` | Read-only, Dev-bound diagnosis of one populated `*_svc` group's existing containers, including stopped state, exit code, and restart count; no environment or log output |
| Provision NetBox Custom Fields | `provision-netbox-custom-fields.yml` | Converge the workflow's NetBox custom fields from `workflows/service-onboarding/netbox-custom-fields.yml` via the Django shell; refuses to retype a field. Has a `(Dev)` copy (the playbook is on dev until promotion); production's fields were created by it on 2026-09-25 |
...
Launch rights are runner rights: a task's environment becomes its extra vars unfiltered,
and a templated extra var can run a command on the runner, so `task_runner` is granted only
to someone trusted with the controller, and `Manage Semaphore Access` holds the team to its
declaration (plan 01, "Launch permission is runner access").
...
- `scripts/semaphore-seed-input.py` / `scripts/postiz-seed-input.py` — stage an operator-held
  secret as an encrypted input in one seed template's isolated environment, run one task, then
  remove the input. The Semaphore token arrives on stdin, never argv; `--apply` is required to
  write and the approved `--inventory` id is required to seed. Both resolve their target and run
  their read-only preflight through `scripts/semaphore_seed.py`. Procedure: the
  [operating guide](platform/semaphore/README.md).
...
- **Semaphore pipeline** — every task template declared in `templates.yml` (plus generated `(Dev)` variants; the count is deliberately not written here, it drifted with every added template), SSH key auth
...
- **Inference edge (prod route LIVE since 2026-09-12, Semaphore task 705; rate limit live 2026-09-15)** — `inference.uhstray.io` fronts vLLM on the DGX Spark pair (spark-1 serves the OpenAI-compatible API; spark-2 is its Ray worker) in the vLLM security-guide allowlist shape: `/v1/*` with an edge 401 for a missing Bearer header, `/health`, 404 for everything else, SSE streaming, 16MB body cap. Proven locally through the Semaphore-driven `deploy-caddy.yml` against `scripts/fake-inference-upstream.py` (200/401/404/413 + per-chunk stream arrival, 2026-09-11). The prod block lives in site-config's `caddy_managed_sites` (merged via site-config#13) and is placed by `Manage Caddy Sites`; verified live 2026-09-15: `/v1/models` without a key answers 401 with no `cf-mitigated`, `/metrics` is challenged at the edge, and the dgx-spark public-path verify play passes (7/7 efforts, 256 s stream, 12/12 queued streams). The Cloudflare record for the hostname pre-exists by hand and is NOT yet in `platform/infra/cloudflare/dns.tf`; the challenge bypass for `/v1/*` + `/health` is the `inference-api-bypass-challenge` skip rule in `waf.tf` (applied 2026-09-12). **Rate limit APPLIED 2026-09-15 (Semaphore tasks 884 plan / 885 apply / 886 zero-diff, via the API); origin lockdown WITHDRAWN 2026-09-15 (landed via site-config#13, failed closed — the Caddy container does not see a Cloudflare peer address — reverted via #14; operator decision: no Cloudflare-range lockdown of the origin in any form):** `ratelimit.tf` adds the zone's first `http_ratelimit` ruleset (Pro tier read from the API: 2 rules max, 10 s period legal) — a per-source `ip.src` ceiling of 10 requests / 10 s on `/v1/*` (counted per Cloudflare data center), rolled out STAGED and now ENFORCING: `log` only at first apply (2026-09-15), `block` (10 s mitigation) enabled the same morning after a paced measurement showed the counter exact for sequential arrivals (11/15/20 requests → 1/5/9 log events) and proven live (15 requests → ten 401, five 429, served again after the window); the log twin is removed, one rule holds —. The origin stays reachable from any peer; the shared Bearer check at Caddy and vLLM's own `--api-key` are the controls for a direct caller. The 125 s Cloudflare read timeout is an accepted decision (`plan/architecture/05-platform-infra.md`, "Cloudflare-proxied hostnames"); the fix is dgx-spark's origin-side SSE heartbeat
