...
| Provision tududi-github Sync | `provision-tududi-github-sync.yml` | Place the whole tududi↔GitHub sync: live-validate BOTH provider credentials (named refusals before any engine write), upsert the two named n8n credentials (domain-pinned), render the cycle workflow from the committed eight-pair mapping + embedded sync core, upsert/activate by name, prune owned (prefix-scoped) objects the declaration no longer implies — an all-disabled mapping prunes everything owned (the specified rollback). Kill switch `-e sync_enabled=false` deactivates BEFORE any validation |
