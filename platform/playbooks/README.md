...
| `inspect-host-containers.yml` | Read-only: every container on one host, per engine, with its restart policy and compose working directory; for onboarding a host with no service group |
