...
| Token lifecycle API | `GET/POST /api/v1/profile/api-keys`, `POST /api/v1/profile/api-keys/{id}/revoke`, `DELETE /api/v1/profile/api-keys/{id}` — session+CSRF authenticated (UI-session surface, the n8n `/rest/api-keys` shape); `createApiToken` accepts `expiresAt` and `abilities` | `frontend/utils/apiKeysService.ts`; `backend/modules/users/apiTokenService.js` |
| **Token mint is automatable** | Yes — session login (the deploy retains break-glass local login beside OIDC) → CSRF token → `POST profile/api-keys`, on the `store-n8n-api-key.yml` model. Design D7's open question: answered | above + `secret/services/tududi` break-glass entry (AGENTS.md secrets table) |
...
cycle: 6 repos × (1 issue-list page + ~1 op per changed pair) ≈ 12–20 calls;
at a 5-minute cadence that is ≈ 150–240 calls/h — under 5% of the ceiling.
