...
_fw(allow_rules) := object.union(_run("security-agent", "Apply Firewall"), {
	"step": "fw-assess",
...
_svc(spec, template) := object.union(_run("service-agent", "Deploy tududi"), {
	"step": "service-assess",
...
test_role_denied_outside_its_allowlist if {
	d := agentcloud.decision with input as _run("o11y-agent", "Apply Firewall")
	not d.allowed
	d.reason == "template is not on this agent's allowed_templates"
}

test_role_allowed_on_its_own_template if {
	agentcloud.allow with input as _run("security-agent", "Apply Firewall")
}
...
test_destructive_template_denied_for_workflow_agent if {
	d := agentcloud.decision with input as _run("security-agent", "Clean Deploy Authentik")
	not d.allowed
...
test_unreviewed_step_denied_on_main if {
	d := agentcloud.decision with input as object.union(_run("security-agent", "Apply Firewall"), {"git_branch": "main"})
	not d.allowed
	d.reason == "an unreviewed step cannot run from main"
}

test_missing_branch_means_main if {
	not agentcloud.allow with input as object.remove(_run("security-agent", "Apply Firewall"), ["git_branch"])
}

test_reviewed_step_allowed_on_main if {
	agentcloud.allow with input as object.union(
		_run("security-agent", "Apply Firewall"),
