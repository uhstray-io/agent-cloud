...
@test "Makefile: local-up invokes local-bootstrap first" {
  # local-up must bring up the genesis before Tier-3 services.
  local bootstrap_line tier3_line
  bootstrap_line=$(grep -n 'local-bootstrap' "$MF" | grep -v '^[0-9]*:.*#\|^[0-9]*:.*\.PHONY' | grep 'local-up' -A5 | head -1 | cut -d: -f1)
  # Simpler: local-up's recipe block contains local-bootstrap before deploy targets
