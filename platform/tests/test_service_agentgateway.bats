...
@test "agentgateway: deploy.sh probes readiness from the sibling db container (image has no shell)" {
  ...
  # By address, never by name: a VM named `agentgateway` resolved the name to the db
  # container itself (task 1215), so these once-pinned URLs were the bug.
  assert_grep -qF 'http://${addr}:19001/healthz/ready' "$DEPLOY_DIR/deploy.sh"
  assert_grep -qF 'gateway-addr.sh' "$DEPLOY_DIR/deploy.sh"
  refute_grep -qF 'http://agentgateway:' "$DEPLOY_DIR/deploy.sh"
...
@test "agentgateway: deploy playbook is composable — manage-secrets, existing upstream key, renders both files" {
  ...
  # The upstream key is never generated by the deploy. The declaration lives where
  # check-secrets can evaluate it too, and the deploy loads it from there.
  local decl="$REPO_ROOT/platform/playbooks/vars/secret-declarations/agentgateway.yml"
  assert_grep -q 'vars/secret-declarations/agentgateway.yml' "$PLAYBOOK"
  assert_grep -qE "'name': 'vllm_api_key', 'type': 'existing'" "$decl"
  assert_grep -qE "'name': 'agw_db_password', 'type': 'random'" "$decl"
  ...
  assert_grep -qE "'name': 'agw_oidc_cookie_seed', 'type': 'random'" "$decl"
...
@test "agentgateway: gateway-addr.sh prints the gateway's address on the db's network" {
  # Task 1215: the VM is named `agentgateway`, podman 4.x seeds container /etc/hosts
  # from the host's, and the db container resolved the service name to itself.
  cat > "$BATS_TEST_TMPDIR/engine" <<'STUB'
#!/usr/bin/env bash
case "$2" in
  agentgateway-db) [ "${NO_NET:-0}" = 1 ] || echo "agentgateway_agentgateway " ;;
  agentgateway) case "$4" in *'"agentgateway_agentgateway"'*) echo "192.0.2.7" ;; esac ;;
esac
STUB
  chmod +x "$BATS_TEST_TMPDIR/engine"
  run env CONTAINER_ENGINE="$BATS_TEST_TMPDIR/engine" "$DEPLOY_DIR/gateway-addr.sh"
  [ "$status" -eq 0 ]
  [ "$output" = "192.0.2.7" ]
  run env CONTAINER_ENGINE="$BATS_TEST_TMPDIR/engine" NO_NET=1 "$DEPLOY_DIR/gateway-addr.sh"
  [ "$status" -ne 0 ]
  assert_contains "$output" "agentgateway-db is on no network"
}
