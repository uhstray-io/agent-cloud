...
| **Internal TLS** — services/operators on `*.agent-cloud.test` / internal zones / LAN | **Caddy internal CA** (`tls internal` / automatic-https internal issuer) — trust the root once (`make local-tls-trust`); **step-ca** (`INTERNAL-CA-DEPLOYMENT.md`) only if a shared root across many hosts is wanted. | Internal-only; LE can't validate non-public names. |
