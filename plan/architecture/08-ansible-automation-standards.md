...
| `command-instead-of-module` | Skipped in `.ansible-lint`, and uses also carry `# noqa` (for example `apply-firewall.yml:160`) | No reason is recorded. `community.general.ufw` and `containers.podman.podman_container` exist on the workstation; whether the controller image has them was not checked. Recorded as debt: a module call simulates under `--check`, a `command` does not |
