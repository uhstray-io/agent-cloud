...
- **Artifacts built:** `.github/rulesets/protect-main.json` (canonical), `.github/rulesets/apply.sh` (idempotent create-or-update), `.github/rulesets/README.md`. The remaining step is the live apply via `apply.sh` by a repository admin, then the `evaluate → active` flip.
...
**Phase 3 — CodeRabbit enforcement (next).** `.coderabbit.yaml` does not yet exist — create it with `reviews.request_changes_workflow: true` so unresolved findings hold the merge via the conversation-resolution rule. Verify whether current CodeRabbit also exposes a check-run that can be added as a required status check.
