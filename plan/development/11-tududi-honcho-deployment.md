...
**Context:** Onboard two new rootless-podman services through the composable pattern — **tududi** (self-hosted to-do app, `todo.uhstray.io`, prod VM `.189`) and **honcho** (ambient memory / identity backend for AI agents, `memory.uhstray.io`, prod VM `.190`) — full IPs live in site-config inventory. Local-dev first (`make local-deploy-*`), then promotion to the two prod VMs with Authentik SSO, Caddy public subdomains, per-service SSH keys and the UFW firewall. tududi becomes the migration sink for NocoDB work data (driven by **weft**); honcho becomes the self-hosted memory backend for the **evolve** skill and team collaboration.
...
- **`templates/env.j2`:**
  ...
  - **LLM (D3 — Gemini interim):** `LLM_GEMINI_API_KEY={{ secrets.gemini_api_key }}`; set each **text-gen** feature to `..._MODEL_CONFIG__TRANSPORT=gemini` + a `..._MODEL_CONFIG__MODEL=<gemini function-calling model>` (deriver, summary, dream, each `DIALECTIC_LEVELS__*`). **Embeddings stay OFF** — `EMBED_MESSAGES=false` and the embedding transport is left unconfigured (no OpenAI key here); the env template enforces this. **Skynet swap later:** flip transport to `openai` + `..._OVERRIDES__BASE_URL=<skynet /v1>` + `..._OVERRIDES__API_KEY_ENV=SKYNET_API_KEY` (config-only; models must support tool calling), then re-enable embeddings and set `EMBEDDING_VECTOR_DIMENSIONS` to match the served model.
...
1. **Inventory (site-config):** add the host under `virtualmachines` (`.189`/`.190` + `service_port`) and the `<svc>_svc` group with `service_name`, `service_url`, `health_path`, `monorepo_deploy_path`, `container_engine: podman`, plus Caddy + firewall vars (`firewall_ssh_cidrs`, `firewall_upstream_source: <central-caddy-host>`; leave `firewall_rootful` false — rootless).
...
- **Risks:** (1) honcho deriver needs a **tool-calling** LLM — verify Gemini function-calling against honcho before relying on derivation. (2) honcho image must be CI-built to GHCR before the prod deploy. (3) VM reachability/size — confirm `.189/.190` respond on the default password and have headroom (honcho). (4) Rootless boot persistence: enable `loginctl enable-linger` for the service user so rootless containers auto-start on reboot (with `restart: always` in compose) — the rootless replacement for the rootful systemd concern. (5) Public `*.uhstray.io` DNS records are a Cloudflare-side step outside the playbooks. (6) **Prod tududi OIDC TLS trust:** tududi's Node backend does server-side OIDC discovery against `auth.uhstray.io`; if that host is served with Caddy's internal CA (`tls internal`) rather than a publicly-trusted cert, discovery fails as it did locally. Before promoting, ensure `auth.uhstray.io` is publicly trusted (Cloudflare DNS-01) or distribute a prod step-ca root to tududi (`NODE_EXTRA_CA_CERTS`, as done locally). Applies to any server-side-OIDC prod client.
...
- [ ] Ship honcho's prod Caddy block as a repo-side `platform/services/honcho/deployment/templates/caddy-site.j2` (mirroring the path-scoped `forward_auth` from `Caddyfile.local.j2` — the header-strip + `@gated /docs*` block) rather than free-handing that security-sensitive block into site-config `caddy_managed_sites`; wire behind `caddy_composable` like openhands.
