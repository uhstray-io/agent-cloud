# 08 — ERPNext
> **Consolidates:** ERPNEXT-DEPLOYMENT.md (originals archived in `plan/archive/`)
>
> **Depends on:** 00, 01, 02, 05
>
...
> **Location:** `plan/development/08-erpnext.md`
...
2. *(repo)* `plan/architecture/01-automation-model.md:229-344` — composable task library; `_secret_definitions`/`_env_templates` schema (§7.7).
3. *(repo)* `plan/architecture/02-service-onboarding.md:55-173` — new-service onboarding checklist (Phases 0–1, §11.1).
4. *(repo)* `plan/architecture/05-platform-infra.md` — engine selection; podman-compose 1.0.6 gaps driving §7.2–§7.3; cross-runtime best practices.
5. *(repo)* `plan/architecture/05-platform-infra.md` — per-site fragment pattern + Cloudflare DNS-01 (§7.6).
6. *(repo)* `plan/architecture/04-credentials-access.md` — storage paths; Create→Verify→Retire rotation (§13).
7. *(repo)* `plan/architecture/04-credentials-access.md:15-76` — Semaphore vs SSH matrix; AppRole scopes; blast-radius table (§18).
8. *(repo)* `plan/architecture/03-testing-ci-quality.md:228-270` — per-service test requirements (§7.10).
9. *(repo)* `plan/architecture/03-testing-ci-quality.md` — secret/IP scan gates; no-`no_log` rule (§20).
10. *(repo)* `plan/development/01-secrets-credentials.md` — callback-plugin redaction replacing `no_log`.
11. *(repo)* `plan/architecture/03-testing-ci-quality.md` — `service_branch` deploy/validate/rollback used by every phase.
12. *(repo)* `plan/architecture/00-foundation-standards.md:15-43` — plan-document standards (status lifecycle, mermaid-only diagrams).
...
19. *(repo)* `plan/development/06-inference-skynet.md` (+ archived `plan/archive/WISAI-DEPLOYMENT-PLAN.md`) — Plane A inference (skynet `/v1`); LiteLLM rejection; `secret/services/inference/*` paths (Phase 2 dependency).
20. *(repo)* `plan/development/07-websmith-uhhcraft.md` — GPU hardware decision gate feeding §2/§9; VRAM constraints.
21. *(repo)* `plan/development/10-infra-resilience.md` — backup/restore strategy and drill cadence (§8.3–§8.4, §13).
22. *(repo)* `plan/development/03-guardrails-governance.md` — policy-as-code option (§13).
23. *(repo)* `plan/development/09-service-migrations-tooling.md` — held legacy-path migration; stateful-secret hazard informing the secrets decision (§3).
