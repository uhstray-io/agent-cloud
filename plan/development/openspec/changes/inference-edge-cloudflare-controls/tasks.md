...
- [ ] 0.1 Feature branch from `dev`: `feat/inference-edge-cloudflare-controls`; one `feat → dev`
      PR, then `dev → main`
- [ ] 0.2 Read the zone plan tier and list existing `http_ratelimit` rules through the
...
- [ ] 0.3 Validation gate: the tier and existing-rule count are written down, and
...
- [ ] 1.1 `platform/infra/cloudflare/ratelimit.tf`: one `cloudflare_ruleset` in phase
...
- [ ] 1.2 If the tier permits a second rule, add a `log` twin first and keep it for one review
      period; otherwise skip and note why in the file
- [ ] 1.3 Update the pointer comment on the inference skip rule in `waf.tf` to name
      `ratelimit.tf`
...
- [ ] 2.1 Inventory variable `caddy_cloudflare_ranges` (list of CIDRs) with the fetch date and
...
- [ ] 2.2 `Caddyfile.local.j2`, `inference_api` branch: a named matcher
...
- [ ] 2.3 Mirror the same matcher into the production block in site-config, using the same
      variable, keeping the block's comment accurate
- [ ] 2.4 `platform/tests/test_service_caddy.bats`: extend the inference test to assert the
...
## 4. Records
- [ ] 4.1 `plan/development/13-cloudflare-iac.md`: status line for the rate-limit rule; the
      `http_ratelimit` phase joins the managed set
- [ ] 4.2 `plan/architecture/` record: the 125 s Proxy Read Timeout is accepted; Enterprise and
      grey-cloud rejected with reasons; heartbeat implemented by dgx-spark
      `inference-endpoint-reliability`
- [ ] 4.3 Cloudflare range refresh procedure documented next to the variable (re-fetch the two
