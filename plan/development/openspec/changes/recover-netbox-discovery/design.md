...
Introduce a validated source mapping with `enabled`, approved disablement reason when false, allowed targets, cron schedule, maximum interval seconds, collection timeout seconds, ingestion deadline seconds, credential-consumption deadline seconds and expected-object selectors. Use the same mapping in agent rendering, the verifier and re-runnable recovery. Require a positive finite credential-consumption deadline for credential-backed sources; credential-free sources explicitly declare zero. Missing fields fail before mutation; strings masquerading as booleans do not become silent opt-outs. Validate correspondence between cron and maximum interval for the supported periodic schedules.
...
Vault polling is currently hourly. Each credential selection records a non-secret selection time and KV version reference. First recovery must prove the running agent consumed that selected version before its declared credential-consumption deadline, using agent consumption evidence linked to subsequent successful source authentication and a cycle. If the installed agent cannot expose version consumption, require an explicitly authorized, recorded reload of that selected version followed by source authentication from that agent; controller authentication alone is insufficient. An old-version cycle, missing evidence or an expired deadline fails acceptance. A retry reuses the original selection time/version and deadline rather than resetting the clock. An explicit agent reload or redeploy is a repair operation, never hidden inside the verifier.

Default verifier polling is five minutes. First-recovery freshness, measured from credential selection, is bounded by `credential-consumption deadline + 2 × source interval + collection timeout + ingestion deadline`. Consumption must independently pass its earlier deadline; the larger cycle budget cannot excuse a late reload. After confirmed consumption, steady-state freshness uses `2 × source interval + collection timeout + ingestion deadline` from the last accepted cycle; do not add refresh slack to every check. Explicit failures are non-healthy immediately; an expired budget becomes stale by the next poll. Validate positive finite bounds, timestamps, and clocks. Confirm backend timeout units on the actual installed versions before converting their existing settings; do not interpret SNMP's current `600` by analogy with nmap's timeout. Observability retention must exceed the longest first-recovery budget plus verifier margin; use bounded log retention and reject missing history as unknown.
