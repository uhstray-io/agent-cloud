...
- [ ] 4.2 Open and merge the local metrics, logs, and SSO baseline PR to `dev`; keep alerts disabled until their destination and drill pass. Submit every PR required for the rollout under the user's standing authorization. For each PR, verify one completed CodeRabbit review round, resolve actionable findings, and confirm all required checks are green on the final head; then merge under that authorization. Run subsequent local and production automation only from the exact reviewed `dev` revision.
