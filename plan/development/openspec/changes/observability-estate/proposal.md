...
- Validate the isolated branch through local Semaphore, including Grafana's Authentik login, TLS, live metrics/log queries, and a notification drill, before opening a PR to `dev`. Submit every PR required for this rollout. Each PR may be merged after one completed CodeRabbit review round, resolution of actionable findings, and all required checks passing on the final head.
