...
- Validate the baseline metrics, logs, TLS, and Grafana Authentik login through local Semaphore before its PR to `dev`. Keep alerts disabled in that baseline. A reversible Semaphore canary temporarily provisions the OpenBao-backed route, proves the rule and Discord receipt, and restores the paused state even when the drill fails. Only a later reviewed rollout persists alert enablement. Submit the PRs required for this rollout under the user's standing authorization. Each PR may be merged after one completed Claude or CodeRabbit review round, resolution of actionable findings, and all required checks passing on the final head.
