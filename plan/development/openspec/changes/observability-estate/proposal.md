...
- Validate the baseline metrics, logs, TLS, and Grafana Authentik login through local Semaphore before its PR to `dev`. Keep alerts disabled in that baseline; require a notification drill and receipt before the later alert-enable rollout. Submit the PRs required for this rollout under the user's standing authorization. Each PR may be merged after one completed CodeRabbit review round, resolution of actionable findings, and all required checks passing on the final head.
