...
- **Per-client keys and limits at the gateway.** `apiKey` policy with keys minted per
  team member or agent role and stored in OpenBao; `localRateLimit` by requests and by
  tokens keyed on the identity, derived from the concurrency ceiling the reliability
  change measures. The shared vLLM key becomes an internal credential the gateway holds.
...
- Files: `platform/services/agentgateway/deployment/{compose.yml, compose.local.yml,
  deploy.sh, templates/env.j2, templates/config.yaml.j2, .gitignore}`,
  ...
  amendment, `plan/architecture/` record.
- site-config: `agentgateway_svc` host; the production Caddy block's upstream; per-key
  identities list (names only; keys in OpenBao).
- OpenBao: `secret/services/agentgateway` with the vLLM upstream key and the minted
  client keys.
- Live: one new VM in the request path; one Caddy redeploy; one client-key rollout to
