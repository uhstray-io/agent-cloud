...
graph TD
    ...
    subgraph GR["Guardrail Layer"]
        ...
        G2["Kyverno (k8s)"]
        G3["OPA (policy)"]
        G4["OpenShell network policies"]
        G5["RBAC, AppRole scoping"]
        G6["Authentik SSO/OIDC"]
        G7["ITSM gating (NetClaw config)"]
        G8["NeMo Guardrails (AI output validation)"]
        G9["AI proposes -> guardrails validate -> automation runs"]
    end

    subgraph AUTO["Automation Layer"]
        AU1["Ansible playbooks, Bash deploy scripts, Python tooling"]
        AU2["Deterministic, idempotent, auditable"]
    end

    subgraph PLAT["Platform Layer"]
        P1["Compose/Podman (single-site prod)"]
        P2["Kubernetes/k0s (multi-site prod)"]
        P3["Proxmox VMs -> k8s nodes (scale path)"]
    end
...
```mermaid
flowchart TD
    SRC["platform/services/&lt;name&gt;/deployment/compose.yml<br/>(Source of truth)"]
    COMPOSE["Compose path:<br/>Ansible templates env files<br/>-> deploy.sh -> compose up<br/>(single-site production, Semaphore orchestrates)"]
    K8S["K8s path:<br/>kompose convert -> platform/k8s/base/&lt;name&gt;/<br/>-> Kustomize overlays (dev/staging/prod/per-site)<br/>-> ArgoCD syncs from git -> ESO injects secrets from OpenBao"]
...
graph TD
    ROOT["platform/k8s/"]
    BASE["base/&lt;service&gt;/<br/>(Generated by kompose, CI step)"]
    BDEP["deployment.yaml"]
    BSVC["service.yaml"]
    BKUST["kustomization.yaml"]
    OVR["overlays/"]
    DEV["dev/<br/>(k3s/kind: NodePort, no replicas)"]
    STG["staging/<br/>(Single-node: ClusterIP, 1 replica)"]
    PRD["prod/<br/>(Multi-node: Ingress, HPA, PDB)"]
    PKUST["kustomization.yaml"]
    PHPA["hpa.yaml"]
    PNET["network-policy.yaml"]
    SITE["site-&lt;name&gt;/<br/>(Per-site overlays for multi-site)"]
    BOOT["bootstrap/<br/>(k0s/kubeadm cluster setup, from open-k8s)"]
...
**Status:** COMPLETE — OpenBao VM provisioned on {{ proxmox_node }}, OpenBao v2.5.2 deployed via Semaphore. All deployment code consolidated into the **agent-cloud monorepo**. Only two repos: agent-cloud (public) + site-config (private).
...
```mermaid
graph TD
    subgraph REPO["agent-cloud/ (PUBLIC monorepo)"]
        ROOT_DOCS["CLAUDE.md, README.md, CONTRIBUTING.md"]

        subgraph PLATFORM["platform/"]
            subgraph SERVICES["services/ (per-service: deployment/ + context/)"]
                OPENBAO_SVC["openbao/ -- compose.yml, deploy.sh, config/, policies/"]
                NETBOX_SVC["netbox/ -- deploy.sh, post-deploy.sh, compose, templates/, workers/"]
                NOCODB_SVC["nocodb/ -- compose.yml, deploy.sh"]
            ...
            subgraph PLAYBOOKS["playbooks/"]
                DEPLOY_PB["deploy-*.yml (per-service + deploy-all)"]
                LIFECYCLE_PB["clean-deploy-*, update-*, validate-*, check-secrets"]
                INFRA_PB["distribute-ssh-keys, harden-ssh, install-docker"]
                PROVISION_PB["provision-vm, provision-template, proxmox-validate"]
                TASKS["tasks/ -- manage-secrets, manage-approle, manage-diode-credentials, deploy-orb-agent, clean-service, clone-and-deploy"]
            end

            LIB["lib/ -- common.sh, bao-client.sh"]
            INV["inventory/ -- local.yml, production.yml (placeholders)"]
            SEMCFG["semaphore/ -- templates.yml, setup-templates.yml"]
            HYPER["hypervisor/proxmox/ -- cloud-init/"]
            K8S["k8s/ -- base/, overlays/, bootstrap/ (Phase 3)"]
        end

        subgraph AGENTS["agents/"]
            NEMO_AGENT["nemoclaw/ -- deployment/, context/"]
            NETCLAW_AGENT["netclaw/ -- deployment/, context/"]
            COWORK_AGENT["cowork/ -- context/"]
...
```mermaid
flowchart TD
    S1["Step 1: Per-service compose files<br/>platform/services/*/deployment/compose.yml<br/>DONE"]
    S2["Step 2: Shared libraries<br/>platform/lib/common.sh + bao-client.sh<br/>DONE"]
    S3["Step 3: Deploy scripts (container ops only)<br/>platform/services/*/deployment/deploy.sh<br/>NetBox: deploy.sh + post-deploy.sh<br/>DONE"]
    S4["Step 4: Composable task library<br/>manage-secrets, manage-approle, manage-diode-credentials,<br/>deploy-orb-agent, clean-service<br/>DONE"]
    S5["Step 5: Provision OpenBao VM via Proxmox API<br/>proxmox-validate, provision-template, provision-vm<br/>DONE (2026-03-28)"]
    S6["Step 6: Deploy services to VMs via Semaphore<br/>NocoDB and n8n need composable deploy playbooks<br/>PENDING"]
    S7["Step 7: Jinja2 env templates for NocoDB + n8n<br/>Create templates/*.j2, define definitions + templates<br/>PENDING"]
    S8["Step 8: deploy.sh + post-deploy.sh split for NocoDB + n8n<br/>Split monolithic deploy.sh<br/>PENDING"]
    S9["Step 9: Semaphore config-as-code<br/>templates.yml + setup-templates.yml (25+ templates)<br/>DONE"]
    S10["Step 10: Sparse checkout + runtime dir tasks<br/>sparse-checkout, setup-runtime-dir, run-deploy, verify-health<br/>PENDING"]
...
- **Validated** against PVE 9.0.3 cluster: API, 11 nodes, storage validated, Ubuntu ISO on SharedISOs, SSH keys, network bridge vmbr0
...
```mermaid
flowchart TD
    S1["Step 1: NocoDB CRUD + task_log table<br/>(foundation for everything)"]
    S2["Step 2: Discord messaging + alert webhook<br/>(needed for error reporting)"]
    S3["Step 3: GitHub issue management<br/>(first real integration)"]
    S4["Step 4: Proxmox monitoring<br/>(second integration)"]
    S5["Step 5: n8n workflow triggering<br/>(builds the orchestration workflows)"]
    S6["Step 6: Semaphore playbook execution<br/>(final integration)"]
    S7["Step 7: Scheduled task framework<br/>(ties everything together)"]
...
```mermaid
flowchart TD
    subgraph DEV["DEV (local workstation / feature branch)"]
        D1["1. Develop Dockerfile / image changes locally"]
        D2["2. Build + tag with dev label: image:dev-sha"]
        D3["3. Test locally (compose up, smoke tests)"]
        D4["4. Iterate until ready"]
    end

    subgraph QA["QA (Semaphore CI pipeline)"]
        Q1["1. Semaphore builds image from merged code"]
        Q2["2. Tag: image:qa-version (semver or date-based)"]
        Q3["3. Push to Harbor: harbor/qa/image:tag"]
        Q4["4. Deploy to QA environment using Harbor image"]
        Q5["5. Run automated validation (health checks, integration tests)"]
        Q6["6. Manual approval gate (if required)"]
    end

    subgraph PROD["PROD (Semaphore deploy pipeline)"]
        P1["1. Retag QA image: harbor/prod/image:version"]
        P2["2. Deploy via Semaphore using harbor/prod/ image"]
        P3["3. Compose/k8s manifests pin to harbor/prod/image:version"]
        P4["4. Post-deploy validation (health checks, service verify)"]
        P5["5. Previous prod image retained for rollback"]
    end

    DEV -- "git push, PR merged" --> QA
    QA -- "promotion (retag, no rebuild)" --> PROD
```
...
- **Rename PVE API token** — update `{{ proxmox_token_id }}` to a descriptive name. Update Proxmox, OpenBao, all playbooks.
