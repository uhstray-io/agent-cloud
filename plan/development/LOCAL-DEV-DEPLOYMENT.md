...
- [ ] **hickory-dns local** (§5.1): `platform/services/dns/` scaffolding shared with `DNS-SERVER-DEPLOYMENT.md` (compose + zone-file `.j2` + composable playbook), "Deploy DNS (Local)" entry in `templates-local.yml`, `/etc/resolver` offer in `local-init`, registry entry `127.0.0.1:5300`
