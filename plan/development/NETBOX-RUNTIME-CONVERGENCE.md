...
The recovery playbook runs from a clean reviewed `dev` checkout in Semaphore. It requires the host's existing Compose file to have the same SHA-256 checksum, then transfers the reviewed inspection script through Ansible's temporary script mechanism. The host checkout is not updated. The inspection compares the existing containers with `docker compose --project-name netbox config`: image references, local image IDs, restart declarations, named-volume names/destinations, bind mounts, and environment. It refuses drift before a container change and reports `recreate`, `start`, or `noop` for each core service. The default Semaphore run reports safe metadata and stops without changing runtime files, containers, or secrets. An explicitly selected apply uses Compose for the three backing services, waits for health, then converges NetBox, with `--no-deps --no-build --pull never` and no `down`. A container with the old `no` restart policy is force-recreated so Compose applies the declared `always` policy; a stopped container already carrying `always` is started with `--no-recreate`, and a healthy matching container is left alone. The post-apply check requires started and untouched containers to keep their IDs. Docker Compose preserves mounted volumes on recreation. Existing OpenBao-rendered env and bind-mount secret files are required and preserved; the recovery does not generate or rewrite credentials. A second run must make no container change.
...
| PR gate | Every PR has one completed review, resolved actionable findings, and green final-head checks; Claude review is accepted when CodeRabbit is rate limited. |
