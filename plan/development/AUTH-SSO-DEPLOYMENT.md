...
| Semaphore | **OIDC** (Authentik side DONE; control-plane side PENDING) | native OIDC via `SEMAPHORE_OIDC_PROVIDERS`. Authentik provider/app/secret done + discovery validated; the Semaphore-side config lands with the **bootstrap reorder** (LOCAL-DEV-DEPLOYMENT §12A) so Semaphore boots OIDC-secured after step-ca/authentik. Malformed OIDC JSON panics startup → jq-validate; local admin is the fallback (no group→role mapping) |
