...
11. **Promotion to production**: once the `dev` changes are validated, open a `dev` → `main` PR. This one **is** squash-merged (keeps `main`'s required linear history). A GitHub Action (`.github/workflows/sync-main-to-dev.yml`) then **auto-reconciles `dev` with `main`** after the merge — so you never hand back-merge again (see "Why the auto-sync" below).
...
**Why the auto-sync (`main` → `dev`).** Squashing the `dev` → `main` promotion writes `dev`'s content onto `main` as a *new* commit with no ancestry into `dev`, freezing the `dev`↔`main` merge-base. The next `dev` → `main` PR then diffs from that stale base and conflicts on any file changed on both sides (e.g. `templates.yml`). The sync workflow runs on every push to `main`, merges `main` back into `dev` favoring `dev` (which is content-ahead — `-X ours`, so dev's content is unchanged and the merge-base just advances; non-conflicting `main`-only changes like dependabot bumps still propagate), and pushes `dev`. This replaces the manual back-merge entirely.

**Enforcement.** On `main` this is no longer convention alone — it is mechanically enforced by the `protect-main` repository ruleset (config-as-code in `.github/rulesets/`): no direct or force pushes, no deletion, PR required, review conversations resolved, and the `Static Analysis` / `Security Scan` / `Unit Tests` checks must pass; merges into `main` are squash-only with linear history. (This squash-only rule governs the `dev` → `main` promotion; it does NOT apply to feature → `dev` merges, which use merge commits per step 10 above. `dev` itself is not push-protected — the sync workflow pushes to it.) (The ruleset currently runs in `evaluate`/dry-run — it logs would-be violations rather than blocking — and flips to `active` after Insights verification; see `.github/rulesets/README.md`.) The sole bypass actor is the Repository admin role (break-glass) — AI agents (NemoClaw, Claude Code) and automation PATs have no bypass path. See `.github/rulesets/README.md` and `plan/development/03-guardrails-governance.md`.
