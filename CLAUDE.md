...
| Set Semaphore Branch | `set-semaphore-branch.yml` | Flip the branch Semaphore runs agent-cloud playbooks from (`dev`↔`main`) via the Semaphore API — the batch-to-dev → test-on-dev → promote-to-main toggle. Keep prod on `main`; flip to `dev` only while testing |
...
**Batch to `dev`, test on `dev`, then promote to `main` (standard practice).** Land multiple feature PRs into `dev`, **validate the integrated `dev` branch** (via Semaphore against real infra and/or local-dev), and only then open ONE `dev` → `main` promotion PR. Don't promote each feature to `main` individually — batch and test first. To test the integrated `dev` in Semaphore, use the **"Set Semaphore Branch"** template (`set-semaphore-branch.yml`) to flip the branch Semaphore runs playbooks from: set it to `dev` while testing, run the deploys/tasks (pass `service_branch=dev` where a deploy should also push dev's monorepo to the VM), then **flip back to `main`** before/after promotion. Prod default is always `main`.
