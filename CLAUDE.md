...
**Batch to `dev`, test on `dev`, then promote to `main` (standard practice).** Land multiple feature PRs into `dev`, **validate the integrated `dev` branch** (via Semaphore against real infra and/or local-dev), and only then open ONE `dev` → `main` promotion PR. Don't promote each feature to `main` individually — batch and test first. To test the integrated `dev` in Semaphore, flip the branch Semaphore runs playbooks from with the **operator-side** `set-semaphore-branch.yml` (run from a workstation with `SEMAPHORE_URL`/`SEMAPHORE_TOKEN`, like `setup-templates.yml`; NOT a Semaphore job — managing Semaphore's own config from inside a job is avoided by convention): `-e deploy_branch=dev` while testing, run the deploys/tasks (pass `service_branch=dev` where a deploy should also push dev's monorepo to the VM), then flip back with `-e deploy_branch=main` before/after promotion. Prod default is always `main`.
